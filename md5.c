...
void send_to_process(int child_index, int quantity, int *files_assigned, int total_files_to_process, ipc_resources *ipc,
                     const char *argv[]);
...
void send_initial_files(program_params *params, ipc_resources *ipc, const char *argv[], int files_per_child) {
    for (int child_index = 0;
         child_index < params->child_qty && params->files_assigned <= params->total_files_to_process; child_index++) {
        send_to_process(child_index, MIN(params->total_files_to_process, files_per_child), &params->files_assigned,
                        params->total_files_to_process, ipc, argv);
    }
}
...
void
print_output(int *printed_files, program_params *params, ipc_resources *ipc, int index, int argc, const char *argv[],
             char *path) {
...
void read_from_slaves(program_params *params, ipc_resources *ipc, int argc, const char *argv[], fd_set *read_fds,
                      int *printed_files) {
...
void
process_remaining_files(program_params *params, ipc_resources *ipc, int argc, const char *argv[], int *printed_files) {
    fd_set read_fds;
...
void close_resources(program_params *params, ipc_resources *ipc, int printed_files) {
    ...
    close_pipes_and_processes(ipc->child_to_parent_pipe, ipc->parent_to_child_pipe, params->child_qty,
                              params->child_pid);
